...
namespace InfuseSync.Storage
{
    public static class SqliteExtensions
    {
        ...
        public static int GetInt(this SqliteDataReader reader, int index)
        {
        ...
        public static long? SelectScalarInt64(this SqliteCommand command)
        {
            var result = command.ExecuteScalar();
            if (result == null || result == DBNull.Value)
            {
        ...
        public static void RunInTransaction(this SqliteConnection This, Action<SqliteConnection> action)
        {
            This.RunInTransaction((Func<SqliteConnection, object>)delegate (SqliteConnection connection)
            {
                action(connection);
                return null;
            });
        ...
        public static T RunInTransaction<T>(this SqliteConnection This, Func<SqliteConnection, T> f)
        {
        ...
        public static bool TableExists(this SqliteConnection This, string name)
        {
            using var statement = This.PrepareStatement("select DISTINCT tbl_name from sqlite_master");
